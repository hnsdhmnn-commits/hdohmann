...
const T = {
  ...
  surfaceWarm:"#FBF9F5", surfaceMid:"#F2EFE8",
  ...
  gold:"#B8922A", goldFaint:"#F5EDD8", goldBorder:"#E8D098", goldDim:"#6B5420",
...
const PAINEL_GENETICO = [
  ...
  {cat:"Perfil Metabólico",icon:"⚗️",color:T.gold,bg:T.goldFaint,genes:["FTO","MC4R","PPARG","ADRB3","TCF7L2","SLC2A2"],desc:"Tendência ao ganho de peso, resistência à insulina e metabolismo."},
...
const INTEGRACOES = [
  {id:"samsung",nome:"Samsung Health",icon:"📱",color:T.blue,plat:["Android"],desc:"Hub nativo Samsung. Galaxy Watch, Ring e dispositivos Samsung.",passos:["Abra o Samsung Health no Android","Emparelhe Galaxy Watch ou Ring via Bluetooth","Configurações → Parceiros → Google Fit → Conectar","HDohmann lê via Google Fit automaticamente"]},
  ...
  {id:"watch",nome:"Apple Watch",icon:"⌚",color:T.blue,plat:["iOS"],desc:"FC, HRV, ECG, SpO2, sono e passos.",passos:["Emparelhe com iPhone pelo app Watch","Ative monitoramento de sono em Saúde → Sono","Ative HRV em Configurações → Privacidade → Saúde","Dados chegam via Apple Health automaticamente"]},
  ...
  {id:"oura",nome:"Oura Ring",icon:"💍",color:T.purple,plat:["iOS","Android"],desc:"Foco em sono, HRV e recuperação.",passos:["Baixe o app Oura e emparelhe via Bluetooth","Use toda noite — coleta principalmente durante o sono","App Oura → Perfil → Integrações → Apple Health","Para Android: Personal Token em cloud.ouraring.com"]},
  ...
  {id:"withings",nome:"Withings Scale",icon:"⚖️",color:T.gold,plat:["iOS","Android"],desc:"Bio-impedância: peso, % gordura e massa muscular.",passos:["Instale Withings Health Mate e configure via Wi-Fi","Pese-se pela manhã antes de comer","Health Mate → Perfil → Apple Health → Sincronizar","HDohmann ajusta metas nutricionais com esses dados"]},
...
function buildPrompt(membro, form, scores) {
  const base = `Perfil: ${form?.nome||"Paciente"}, ${form?.cargo||"Executivo"}, ${form?.idade||"—"} anos. Condições: ${(form?.condicoes||[]).join(", ")||"nenhuma"}. Medicamentos: ${(form?.meds||[]).join(", ")||"nenhum"}. Sono: ${form?.sono||7}h qualidade ${form?.qualSono||5}/10. Treino: ${form?.freqTreino||0}x/sem. Estresse: ${form?.estresse||5}/10. Score: ${scores.total}/100.`;
  ...
  if (membro==="geneticista") return `Você é a Dra. Clara, geneticista da equipe HDohmann. Interprete laudos genéticos com precisão. Conecte achados ao plano de cuidado. Para riscos elevados, informe que o Dr. Dohmann será notificado. ${base}`;
  return base;
}
...
function Lbl({children,color}){
  return <div style={{fontSize:9,letterSpacing:"0.18em",color:color||T.inkLight,textTransform:"uppercase",marginBottom:8,fontFamily:T.fB,fontWeight:600}}>{children}</div>;
}
...
function Card({children,style={},onClick,hover=false}){
  ...
  return (
    <div onClick={onClick}
      onMouseOver={()=>hover&&setHov(true)}
      onMouseOut={()=>hover&&setHov(false)}
  ...
  return (
    <div>
      ...
      <div style={{position:"relative"}}>
        <input autoFocus={autoFocus} type={type} placeholder={placeholder} value={value||""}
...
function Chip({label,active,color=T.gold,bg,onClick}){
  return (
    <button onClick={onClick}
      style={{padding:"8px 14px",borderRadius:6,cursor:"pointer",fontFamily:T.fB,fontSize:12,background:active?(bg||T.goldFaint):"transparent",border:`1.5px solid ${active?color:T.border}`,color:active?color:T.inkMid,transition:"all 0.18s"}}>
      {label}
    </button>
  );
}
...
// ─── Chat IA — componente independente ───────────────────────────
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{flex:1,overflowY:"auto",padding:"24px 28px 8px"}}>
        {msgs.map((msg,i)=>{
          ...
          return (
            <div key={i} style={{display:"flex",flexDirection:isUser?"row-reverse":"row",gap:12,marginBottom:20,alignItems:"flex-start"}}>
              ...
              <div style={{maxWidth:"74%",padding:"14px 18px",background:isUser?T.goldFaint:T.surface,border:`1px solid ${isUser?T.goldBorder:T.border}`,borderRadius:isUser?"16px 16px 4px 16px":"4px 16px 16px 16px",fontSize:13,color:T.ink,lineHeight:1.8,whiteSpace:"pre-wrap",boxShadow:T.shadowCard}}>
                {msg.loading
                  ?<span style={{display:"inline-flex",gap:5}}>{[0,1,2].map(j=><span key={j} style={{width:6,height:6,borderRadius:"50%",background:eq?.cor||T.gold,display:"inline-block",animation:`blink 1.2s ease ${j*0.2}s infinite`}}/>)}</span>
                  :msg.content}
              </div>
            </div>
          );
        ...
        <div style={{padding:"0 28px 14px",display:"flex",gap:8,flexWrap:"wrap",flexShrink:0}}>
          {sugestoes.map((s,i)=>(
            <button key={i} onClick={()=>send(s)}
              style={{padding:"8px 16px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:20,fontSize:12,color:T.inkMid,cursor:"pointer",fontFamily:T.fB,transition:"all 0.18s",boxShadow:T.shadowCard}}
      ...
      <div style={{borderTop:`1px solid ${T.border}`,padding:"14px 28px",display:"flex",gap:10,alignItems:"flex-end",flexShrink:0,background:T.bgWarm}}>
        <textarea ref={inputRef} rows={1}
          placeholder={apiKey?placeholder:"Configure a API Key no painel..."}
          value={input}
  ...
  const handleSubmit = async() => {
    ...
    try {
      if (mode==="register"){
        ...
        if (!userData){setErr("E-mail não encontrado.");setLoading(false);return;}
      ...
      <div style={{width:"44%",flexShrink:0,background:T.surface,borderRight:`1px solid ${T.border}`,padding:"52px",display:"flex",flexDirection:"column",justifyContent:"space-between",boxShadow:"4px 0 24px rgba(60,50,30,0.06)"}}>
        <div>
          <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:52}}>
            <span style={{fontFamily:T.fD,fontSize:38,color:T.ink,lineHeight:1}}>H</span>
          ...
          <div style={{display:"flex",gap:16,marginTop:12}}>
            {EQUIPE.map(e=>(
              <div key={e.id} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
                ...
                <span style={{fontSize:9,color:T.inkLight,textAlign:"center"}}>{e.nome}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:40}}>
        <div style={{width:"100%",maxWidth:420}}>
          ...
          <div style={{fontSize:13,color:T.inkMid,lineHeight:1.8,marginBottom:36}}>
            {mode==="login"?"Acesse seu plano de cuidado personalizado.":"Junte-se à equipe HDohmann."}
          </div>
          <Card style={{padding:"32px"}}>
            ...
            <div style={{marginTop:20,textAlign:"center"}}>
              <button onClick={()=>{setMode(mode==="login"?"register":"login");setErr("");}}
                style={{background:"none",border:"none",fontSize:12,color:T.inkLight,cursor:"pointer",fontFamily:T.fB,textDecoration:"underline"}}>
        ...
        <Card style={{padding:"0",overflow:"hidden"}}>
          ...
          <div style={{padding:"20px 32px",borderBottom:`1px solid ${T.border}`,background:T.bgWarm}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {[
                {icon:"🤖",text:"Conecta o app à IA que dá vida à sua equipe HDohmann"},
                {icon:"🔒",text:"Fica salva apenas no seu dispositivo — não é compartilhada"},
...
function ScreenBoasVindas({onStart}){
  ...
  const slides = [
    {icon:"🏥",titulo:"Saúde não é ausência de doença",texto:"O HDohmann existe para desenvolver sua saúde e produtividade — não apenas tratar problemas quando surgem. É um serviço complementar ao seu médico atual.",cor:T.gold},
    {icon:"👥",titulo:"Sua equipe de saúde dedicada",texto:"Ana coordena seu plano. O Coach te motiva diariamente. Rafael cuida dos seus medicamentos. A Dra. Clara interpreta seu DNA. Todos coordenados pelo Dr. Dohmann.",cor:T.teal},
    {icon:"🧬",titulo:"Personalizado até o DNA",texto:"Seu plano usa seus dados genéticos, biométricos, estilo de vida e histórico clínico para criar recomendações únicas — não para um paciente genérico.",cor:T.purple},
    {icon:"📊",titulo:"Seus dados sempre atualizados",texto:"O plano se renova de 3 formas: automaticamente pelos seus gadgets, quando você envia novos exames e pelo check-in diário rápido com a enfermeira Ana.",cor:T.green},
    {icon:"⏱",titulo:"5 minutos para começar",texto:"Agora vamos montar seu perfil. São 7 etapas simples — quanto mais você preencher, mais personalizado será seu plano.",cor:T.red},
  ];
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:T.fB,padding:40}}>
      <div style={{width:"100%",maxWidth:560}}>
        <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:48,justifyContent:"center"}}>
          <span style={{fontFamily:T.fD,fontSize:32,color:T.ink}}>H</span>
          <span style={{fontFamily:T.fD,fontSize:32,color:T.gold}}>Dohmann</span>
        </div>
          ...
          <div style={{padding:"0 40px 28px",display:"flex",justifyContent:"center",gap:8}}>
            {slides.map((_,i)=>(
              <div key={i} onClick={()=>setSlide(i)}
                style={{width:i===slide?24:8,height:8,borderRadius:4,background:i===slide?s.cor:T.border,transition:"all 0.3s",cursor:"pointer"}}/>
            ))}
          </div>
        </Card>
...
function ScreenOnboarding({user,onComplete}){
  ...
  const [f,setF] = useState({
    nome:user.name,cargo:"",setor:"",idade:"",peso:"",altura:"",horasTrab:50,horasDescanso:2,
    condicoes:[],condicaoOutro:"",meds:[],medOutro:"",alergiaMed:"",energia:6,qualidadeVida:6,sintomas:[],sintomaOutro:"",
    histCardioFam:null,histCancerFam:null,histDiabetesFam:null,histDepressaoFam:null,cirurgias:"",hospitalizacoes:"",
    colesterolTotal:"",colesterolLDL:"",colesterolHDL:"",triglicerides:"",glicoseJejum:"",hemoglobina:"",pressaoSistolica:"",pressaoDiastolica:"",
    sono:7,qualSono:6,exercicios:[],exercicioOutro:"",freqTreino:3,alcool:"",estresse:6,meditacao:-1,dieta:"",tabaco:"",
  ...
  const METAS_LIST = [
    {id:"energia",icon:"⚡",label:"Mais energia e disposição"},
    {id:"foco",icon:"🎯",label:"Foco e cognição"},
    {id:"peso",icon:"⚖️",label:"Composição corporal"},
    {id:"longevidade",icon:"∞",label:"Longevidade e prevenção"},
    {id:"estresse",icon:"🌿",label:"Gestão do estresse"},
    {id:"sono",icon:"◑",label:"Qualidade do sono"},
    {id:"performance",icon:"▲",label:"Performance atlética"},
    {id:"libido",icon:"♦",label:"Saúde hormonal"},
  ];
  ...
  const panels = [
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <TxtInput label="Nome completo" placeholder="Ricardo Costa" value={f.nome} onChange={v=>set("nome",v)}/>
        <TxtInput label="Idade" placeholder="47" type="number" value={f.idade} onChange={v=>set("idade",v)} unit="anos"/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <TxtInput label="Cargo" placeholder="CEO" value={f.cargo} onChange={v=>set("cargo",v)}/>
        <TxtInput label="Setor" placeholder="Financeiro" value={f.setor} onChange={v=>set("setor",v)}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <TxtInput label="Peso" placeholder="82" type="number" value={f.peso} onChange={v=>set("peso",v)} unit="kg"/>
        <TxtInput label="Altura" placeholder="178" type="number" value={f.altura} onChange={v=>set("altura",v)} unit="cm"/>
      </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div>
        <Lbl>Condições diagnosticadas</Lbl>
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {CONDICOES.map(c=><Chip key={c} label={c} color={T.teal} bg={T.tealBg} active={(f.condicoes||[]).includes(c)} onClick={()=>tog("condicoes",c)}/>)}
        </div>
        {(f.condicoes||[]).includes("Outros")&&<div style={{marginTop:12}}><TxtInput placeholder="Descreva outras condições..." value={f.condicaoOutro} onChange={v=>set("condicaoOutro",v)}/></div>}
      </div>
      <div>
        <Lbl>Sintomas frequentes (últimos 3 meses)</Lbl>
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {SINTOMAS.map(s=><Chip key={s} label={s} color={T.orange} bg={T.orangeBg} active={(f.sintomas||[]).includes(s)} onClick={()=>tog("sintomas",s)}/>)}
        </div>
        {(f.sintomas||[]).includes("Outros")&&<div style={{marginTop:12}}><TxtInput placeholder="Descreva outros sintomas..." value={f.sintomaOutro} onChange={v=>set("sintomaOutro",v)}/></div>}
      </div>
      <div>
        <Lbl>Medicamentos em uso</Lbl>
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {MEDS.map(m=><Chip key={m} label={m} color={T.teal} bg={T.tealBg} active={(f.meds||[]).includes(m)} onClick={()=>tog("meds",m)}/>)}
        </div>
        {(f.meds||[]).includes("Outros")&&<div style={{marginTop:12}}><TxtInput placeholder="Descreva outros medicamentos..." value={f.medOutro} onChange={v=>set("medOutro",v)}/></div>}
      </div>
      <TxtInput label="Alergia a medicamentos?" placeholder="Descreva alergias ou deixe em branco" value={f.alergiaMed} onChange={v=>set("alergiaMed",v)}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            {label:"Doenças cardiovasculares",key:"histCardioFam",cor:T.red},
            {label:"Câncer",key:"histCancerFam",cor:T.purple},
            {label:"Diabetes",key:"histDiabetesFam",cor:T.orange},
            {label:"Depressão / ansiedade",key:"histDepressaoFam",cor:T.blue},
          ].map(item=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <TxtInput label="Cirurgias anteriores" placeholder="Descreva ou 'Nenhuma'" value={f.cirurgias} onChange={v=>set("cirurgias",v)}/>
        <TxtInput label="Internações anteriores" placeholder="Descreva ou 'Nenhuma'" value={f.hospitalizacoes} onChange={v=>set("hospitalizacoes",v)}/>
      </div>
      <div>
        <Lbl>Resultados de exames recentes (se disponível)</Lbl>
        <Card style={{padding:"18px 20px",background:T.bgWarm}}>
          <div style={{fontSize:12,color:T.inkMid,marginBottom:16,lineHeight:1.7}}>Preencha os valores que lembrar — não é obrigatório. Você também pode enviar o PDF completo em Documentos.</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
            <TxtInput label="Colesterol Total" placeholder="ex: 185" type="number" value={f.colesterolTotal} onChange={v=>set("colesterolTotal",v)} unit="mg/dL"/>
            <TxtInput label="LDL" placeholder="ex: 110" type="number" value={f.colesterolLDL} onChange={v=>set("colesterolLDL",v)} unit="mg/dL"/>
            <TxtInput label="HDL" placeholder="ex: 55" type="number" value={f.colesterolHDL} onChange={v=>set("colesterolHDL",v)} unit="mg/dL"/>
            <TxtInput label="Triglicerídeos" placeholder="ex: 120" type="number" value={f.triglicerides} onChange={v=>set("triglicerides",v)} unit="mg/dL"/>
            <TxtInput label="Glicose em jejum" placeholder="ex: 92" type="number" value={f.glicoseJejum} onChange={v=>set("glicoseJejum",v)} unit="mg/dL"/>
            <TxtInput label="Hemoglobina glicada" placeholder="ex: 5.4" type="number" value={f.hemoglobina} onChange={v=>set("hemoglobina",v)} unit="%"/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:12}}>
            <TxtInput label="Pressão Sistólica" placeholder="ex: 120" type="number" value={f.pressaoSistolica} onChange={v=>set("pressaoSistolica",v)} unit="mmHg"/>
            <TxtInput label="Pressão Diastólica" placeholder="ex: 80" type="number" value={f.pressaoDiastolica} onChange={v=>set("pressaoDiastolica",v)} unit="mmHg"/>
          </div>
        </Card>
      </div>
    </div>,
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <SldInput label="Sono por noite" value={f.sono} onChange={v=>set("sono",v)} min={4} max={10} unit="h" color={T.purple}/>
        <SldInput label="Qualidade do sono" value={f.qualSono} onChange={v=>set("qualSono",v)} min={1} max={10} unit="/10" color={T.purple}/>
      </div>
      <div>
        <Lbl>Atividades físicas praticadas</Lbl>
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {ATIVIDADES.map(e=><Chip key={e} label={e} color={T.teal} bg={T.tealBg} active={(f.exercicios||[]).includes(e)} onClick={()=>tog("exercicios",e)}/>)}
        </div>
        {(f.exercicios||[]).includes("Outros")&&<div style={{marginTop:12}}><TxtInput placeholder="Descreva outras atividades..." value={f.exercicioOutro} onChange={v=>set("exercicioOutro",v)}/></div>}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div>
          <Lbl>Consumo de álcool</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {["Nunca","Ocasional","Semanal","Diário"].map(a=><Chip key={a} label={a} active={f.alcool===a} onClick={()=>set("alcool",a)}/>)}
          </div>
        </div>
        <div>
          <Lbl>Tabaco</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {["Nunca fumei","Ex-fumante","Fumante ocasional","Fumante regular"].map(t=><Chip key={t} label={t} active={f.tabaco===t} onClick={()=>set("tabaco",t)}/>)}
          </div>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div>
          <Lbl>Padrão alimentar</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {["Onívoro","Mediterrâneo","Low-carb","Cetogênico","Vegetariano","Vegano","Sem padrão"].map(d=><Chip key={d} label={d} active={f.dieta===d} onClick={()=>set("dieta",d)}/>)}
          </div>
        </div>
        <div>
          <Lbl>Meditação / mindfulness</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {["Não pratico","Às vezes","Regularmente"].map((o,i)=><Chip key={o} label={o} active={f.meditacao===i} onClick={()=>set("meditacao",i)}/>)}
          </div>
        </div>
      </div>
    </div>,
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div>
        <Lbl>Objetivos principais (até 3)</Lbl>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4}}>
          {METAS_LIST.map(m=>{
            const active=(f.metas||[]).includes(m.id);
            const limit=(f.metas||[]).length>=3&&!active;
            return (
              <button key={m.id} onClick={()=>!limit&&tog("metas",m.id)}
                style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:active?T.goldFaint:T.bgWarm,border:`1.5px solid ${active?T.gold:T.border}`,borderRadius:10,cursor:limit?"not-allowed":"pointer",opacity:limit?0.4:1,transition:"all 0.18s",fontFamily:T.fB,textAlign:"left",boxShadow:T.shadowCard}}>
                <span style={{fontSize:20}}>{m.icon}</span>
                <span style={{fontSize:12,color:active?T.gold:T.inkMid,lineHeight:1.3}}>{m.label}</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div>
          <Lbl>Como prefere ser acompanhado?</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {["Check-ins diários","Relatórios semanais","Alertas sob demanda","Coaching ativo"].map(c=><Chip key={c} label={c} color={T.teal} bg={T.tealBg} active={f.acompanhamento===c} onClick={()=>set("acompanhamento",c)}/>)}
          </div>
        </div>
        <div>
          <Lbl>Horizonte de resultado esperado</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {["1 mês","3 meses","6 meses","1 ano","Longo prazo"].map(h=><Chip key={h} label={h} active={f.horizonte===h} onClick={()=>set("horizonte",h)}/>)}
          </div>
        </div>
      </div>
    </div>,
    ...
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div>
        <Lbl>Gadgets e dispositivos de saúde</Lbl>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4}}>
          {GADGETS_OB.map(g=>{
            const active=(f.gadgets||[]).includes(g);
            return (
              <button key={g} onClick={()=>tog("gadgets",g)}
                style={{display:"flex",alignItems:"center",gap:10,padding:"13px 16px",background:active?T.tealBg:T.bgWarm,border:`1.5px solid ${active?T.teal:T.border}`,borderRadius:10,cursor:"pointer",transition:"all 0.18s",fontFamily:T.fB,boxShadow:T.shadowCard}}>
                <span style={{fontSize:13,color:active?T.teal:T.ink,fontWeight:500}}>{g}</span>
                {active&&<span style={{marginLeft:"auto",color:T.teal,fontSize:14}}>✓</span>}
              </button>
            );
          })}
        </div>
      ...
      <Card style={{padding:"20px 22px",background:T.goldFaint,border:`1px solid ${T.goldBorder}`}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12,marginTop:4}}>
          {[
            {icon:"⌚",titulo:"Gadgets conectados",desc:"Apple Watch, Oura, Garmin e outros enviam dados em tempo real. O plano se renova diariamente."},
            {icon:"📄",titulo:"Upload de documentos",desc:"Quando você enviar novos exames ou laudos, a equipe lê e incorpora ao plano automaticamente."},
            {icon:"✅",titulo:"Check-in diário com Ana",desc:"Todo dia, a enfermeira Ana faz um check-in rápido de 2 minutos para refinar seu plano."},
          ].map((item,i)=>(
            <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              <span style={{fontSize:20,flexShrink:0}}>{item.icon}</span>
              <div>
                <div style={{fontSize:13,color:T.ink,fontWeight:600,marginBottom:3}}>{item.titulo}</div>
                <div style={{fontSize:12,color:T.inkMid,lineHeight:1.7}}>{item.desc}</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:24}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontFamily:T.fD,fontSize:24,color:T.ink,marginBottom:8}}>Análise Genômica HDohmann</div>
        <div style={{fontSize:13,color:T.inkMid,lineHeight:1.9,maxWidth:480,margin:"0 auto"}}>
          Conheça seu DNA e transforme as informações em ações concretas. Examina mais de 30 variantes em 6 categorias clínicas.
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
        {[
          {icon:"📦",title:"Kit em casa",desc:"Receba o material pelos Correios em 3–5 dias úteis"},
          {icon:"🧪",title:"Coleta simples",desc:"Swab oral — 2 minutos, sem dor"},
          {icon:"📊",title:"Laudo em 30 dias",desc:"PDF analisado pela Dra. Clara"},
        ].map((fi,i)=>(
      ...
      <Card style={{padding:"22px",background:T.goldFaint,border:`1px solid ${T.goldBorder}`}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[
            ...
            {n:"03",t:"Colete",d:"Esfregue firmemente em cada bochecha por 30 segundos"},
            {n:"04",t:"Embale",d:"Insira no tubo, feche e agite 5 segundos"},
            {n:"05",t:"Envie",d:"Qualquer agência dos Correios — frete pago"},
            ...
            <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              ...
              <div>
                <span style={{fontSize:12,color:T.ink,fontWeight:600}}>{s.t} — </span>
                <span style={{fontSize:12,color:T.inkMid}}>{s.d}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <div style={{display:"flex",gap:12}}>
          <button onClick={()=>set("querKit",true)}
            style={{flex:1,padding:"16px",background:f.querKit===true?T.goldFaint:"transparent",border:`1.5px solid ${f.querKit===true?T.gold:T.border}`,borderRadius:10,cursor:"pointer",fontFamily:T.fB,fontSize:12,color:f.querKit===true?T.gold:T.inkMid,transition:"all 0.18s",boxShadow:T.shadowCard}}>
            ✓ Sim, quero solicitar o kit
          </button>
          <button onClick={()=>set("querKit",false)}
            style={{flex:1,padding:"16px",background:f.querKit===false?T.surfaceMid:"transparent",border:`1.5px solid ${f.querKit===false?T.borderMid:T.border}`,borderRadius:10,cursor:"pointer",fontFamily:T.fB,fontSize:12,color:T.inkMid,transition:"all 0.18s",boxShadow:T.shadowCard}}>
            Não por enquanto
          </button>
        </div>
        {f.querKit===true&&(
          <div style={{marginTop:12,padding:"12px 16px",background:T.tealBg,borderRadius:8,border:`1px solid ${T.teal}30`}}>
            <div style={{fontSize:12,color:T.teal,lineHeight:1.7}}>✓ Ótimo! Após concluir o cadastro, a equipe HDohmann entrará em contato para confirmar o endereço.</div>
          </div>
        )}
      </div>
    </div>,
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",background:T.bg,fontFamily:T.fB,color:T.ink}}>
      <div style={{width:220,flexShrink:0,borderRight:`1px solid ${T.border}`,padding:"32px 24px",display:"flex",flexDirection:"column",background:T.surface,boxShadow:"2px 0 12px rgba(60,50,30,0.04)"}}>
        <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:44}}>
          <span style={{fontFamily:T.fD,fontSize:24,color:T.ink}}>H</span>
          <span style={{fontFamily:T.fD,fontSize:24,color:T.gold}}>Dohmann</span>
        </div>
            ...
            return (
              <div key={i} style={{display:"flex",gap:14,alignItems:"flex-start",padding:"9px 0",cursor:done?"pointer":"default"}} onClick={()=>done&&setStep(i)}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
                  <div style={{width:24,height:24,borderRadius:"50%",border:`2px solid ${done?T.green:active?T.gold:T.border}`,background:done?T.green:active?T.goldFaint:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:done?"#FFF":active?T.gold:T.inkFaint,fontWeight:700,transition:"all 0.3s"}}>{done?"✓":i+1}</div>
        ...
        <div style={{paddingTop:20,borderTop:`1px solid ${T.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
            <span style={{fontSize:10,color:T.inkFaint}}>PROGRESSO</span>
            <span style={{fontSize:12,color:T.inkMid,fontWeight:600}}>{pct}%</span>
          </div>
          <div style={{height:3,background:T.surfaceMid,borderRadius:2,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${T.gold},${T.teal})`,transition:"width 0.5s ease"}}/>
          </div>
        </div>
      </div>
        ...
        <div style={{maxWidth:640,margin:"0 auto"}}>
          <div style={{marginBottom:28}}>
            <div style={{display:"flex",gap:10,marginBottom:10,alignItems:"center"}}>
              <span style={{fontSize:12,letterSpacing:"0.2em",color:T.gold,fontWeight:700}}>{`0${step+1}`}</span>
              <span style={{fontSize:11,color:T.inkLight,letterSpacing:"0.15em"}}>— {OB_STEPS[step].toUpperCase()}</span>
            </div>
...
// ─── Módulos do App — cada um é componente próprio ────────────────

function ModuloDashboard({form,scores,setModulo}){
      ...
      <div style={{maxWidth:980,margin:"0 auto",display:"flex",flexDirection:"column",gap:20,animation:"fadeUp 0.4s ease"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            ...
            <div style={{fontSize:13,color:T.inkMid}}>Sua equipe HDohmann está ativa e acompanhando seu plano.</div>
          </div>
            ...
            {Object.entries(scores.eixos).map(([n,sc],i)=>(
              <Card key={i} style={{padding:"16px"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                  <span style={{fontSize:11,color:T.inkMid}}>{n}</span>
                  <span style={{fontSize:20,color:AXIS_COLORS[n],fontFamily:T.fD,fontWeight:700}}>{sc}</span>
                </div>
                <div style={{height:4,background:T.surfaceMid,borderRadius:2,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${sc}%`,background:AXIS_COLORS[n],transition:"width 1.2s ease"}}/>
                </div>
        ...
        <Card style={{padding:"20px 24px",background:T.blueBg,border:`1px solid ${T.blue}30`}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>
            {[
              {icon:"⌚",titulo:"Gadgets",desc:"Apple Watch, Oura, Garmin enviam dados em tempo real. Atualização diária automática.",cor:T.teal},
              {icon:"📄",titulo:"Documentos",desc:"Envie novos exames — a equipe lê e incorpora ao plano imediatamente.",cor:T.purple},
              {icon:"✅",titulo:"Check-in diário",desc:"A enfermeira Ana faz um check-in rápido todo dia para refinar seu plano.",cor:T.green},
            ].map((item,i)=>(
              <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                ...
                <div>
                  <div style={{fontSize:12,color:T.ink,fontWeight:600,marginBottom:4}}>{item.titulo}</div>
                  <div style={{fontSize:11,color:T.inkMid,lineHeight:1.6}}>{item.desc}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            {EQUIPE.map(e=>(
              <Card key={e.id} hover onClick={()=>setModulo(e.id==="coach"?"coach":e.id==="farmaceutico"?"farmaceutico":e.id==="geneticista"?"geneticista":"plano")} style={{padding:"18px"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:5,marginTop:12}}>
                  <div style={{width:7,height:7,borderRadius:"50%",background:T.green,boxShadow:`0 0 6px ${T.green}60`}}/>
                  <span style={{fontSize:9,color:T.green,letterSpacing:"0.12em"}}>ONLINE</span>
                </div>
              </Card>
            ))}
          </div>
        ...
        <Card style={{padding:"20px 24px",background:T.goldFaint,border:`1px solid ${T.goldBorder}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontFamily:T.fD,fontSize:20,color:T.ink}}>Foco em <span style={{color:AXIS_COLORS[lowAxis[0]]}}>{lowAxis[0]}</span> — score: {lowAxis[1]}/100</div>
          </div>
...
function ModuloPlano({form,scores,setModulo}){
      ...
      <div style={{maxWidth:1000,margin:"0 auto",animation:"fadeUp 0.4s ease"}}>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:24}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:T.inkFaint}}>Coordenado pela enfermeira Ana · {new Date().toLocaleDateString("pt-BR")}</div>
          </div>
        ...
        <Card style={{padding:"24px",marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div style={{height:6,width:120,background:T.surfaceMid,borderRadius:3,overflow:"hidden"}}>
                <div style={{height:"100%",width:`${todos.length?(todosDone/todos.length)*100:0}%`,background:`linear-gradient(90deg,${T.gold},${T.green})`,transition:"width 0.4s ease"}}/>
              </div>
            </div>
          </div>
            ...
            {todos.map(todo=>{
              ...
              return (
                <div key={todo.id} onClick={()=>toggleTodo(todo.id)}
                  style={{display:"flex",gap:12,alignItems:"center",padding:"12px 16px",background:done?T.surfaceMid:T.bgWarm,border:`1px solid ${done?T.border:todo.corBg}`,borderRadius:8,cursor:"pointer",transition:"all 0.2s",opacity:done?0.6:1}}>
                  <div style={{width:22,height:22,borderRadius:6,border:`2px solid ${done?T.green:todo.cor}`,background:done?T.green:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.2s"}}>
              ...
              {Object.entries(scores.eixos).map(([n,sc],i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div style={{flex:1,height:5,background:T.surfaceMid,borderRadius:3,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${sc}%`,background:AXIS_COLORS[n],borderRadius:3}}/>
                  </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16}}>
          <Card style={{padding:"20px"}}>
            ...
            {[
              {l:"Nome",v:form?.nome||"—"},
              {l:"Cargo",v:`${form?.cargo||"—"} · ${form?.setor||"—"}`},
              {l:"Sono",v:`${form?.sono||7}h · qualidade ${form?.qualSono||5}/10`},
              {l:"Treino",v:`${form?.freqTreino||0}x/sem`},
              {l:"Estresse",v:`${form?.estresse||5}/10`},
            ].map((it,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",paddingBottom:9,marginBottom:9,borderBottom:i<4?`1px solid ${T.border}`:"none"}}>
                <span style={{fontSize:10,color:T.inkFaint}}>{it.l}</span>
                <span style={{fontSize:12,color:T.ink,fontWeight:500}}>{it.v}</span>
              </div>
            ))}
          </Card>
          <Card style={{padding:"20px"}}>
            ...
            {(form?.meds||[]).filter(m=>m!=="Nenhum").length>0&&(
              <div style={{marginTop:10,padding:"10px 12px",background:T.redBg,borderRadius:6,border:`1px solid ${T.red}30`}}>
                <div style={{fontSize:11,color:T.red,lineHeight:1.6}}>⚠️ Rafael verificará interações.</div>
              </div>
            )}
          </Card>
          <Card style={{padding:"20px"}}>
            ...
            <div style={{marginBottom:14}}>
              ...
              {(form?.condicoes||[]).filter(c=>c!=="Nenhuma").length===0
                ?<div style={{fontSize:12,color:T.inkFaint}}>Nenhuma.</div>
                :(form?.condicoes||[]).filter(c=>c!=="Nenhuma").map((c,i)=><div key={i} style={{fontSize:12,color:T.inkMid,padding:"5px 0",borderBottom:`1px solid ${T.border}`}}>◆ {c}</div>)
              }
            </div>
            {form?.glicoseJejum&&<div style={{fontSize:11,color:T.inkMid}}>Glicose: <strong>{form.glicoseJejum} mg/dL</strong></div>}
            {form?.colesterolTotal&&<div style={{fontSize:11,color:T.inkMid}}>Colesterol: <strong>{form.colesterolTotal} mg/dL</strong></div>}
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{borderBottom:`1px solid ${T.border}`,padding:"16px 28px",display:"flex",alignItems:"center",gap:14,background:T.surface,flexShrink:0,boxShadow:T.shadowCard}}>
        ...
        <div>
          <div style={{fontFamily:T.fD,fontSize:20,color:T.ink}}>Coach de Saúde</div>
          <div style={{fontSize:9,color:T.green,letterSpacing:"0.15em"}}>● ONLINE · IA BASEADA NO SEU PERFIL</div>
        </div>
        <div style={{marginLeft:"auto",display:"flex",gap:20}}>
          {[{l:"VITALIDADE",v:`${scores.total}/100`,c:T.gold},{l:"FOCO",v:lowAxis[0],c:T.red}].map((it,i)=>(
            <div key={i} style={{textAlign:"right"}}>
              <div style={{fontSize:8,color:T.inkFaint,letterSpacing:"0.12em"}}>{it.l}</div>
              <div style={{fontSize:14,color:it.c,fontFamily:T.fD,fontWeight:700}}>{it.v}</div>
            </div>
          ))}
        </div>
      </div>
      <ChatIA membro="coach" apiKey={apiKey}
        placeholder="Fale com seu coach sobre saúde e bem-estar..."
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{borderBottom:`1px solid ${T.border}`,padding:"16px 28px",display:"flex",alignItems:"center",gap:14,background:T.surface,flexShrink:0,boxShadow:T.shadowCard}}>
        <div style={{width:42,height:42,borderRadius:"50%",background:eq.bg,border:`1.5px solid ${eq.cor}40`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,boxShadow:T.shadowCard}}>{eq.icon}</div>
        <div>
          <div style={{fontFamily:T.fD,fontSize:20,color:T.ink}}>{eq.nome} · Farmacêutico Clínico</div>
          <div style={{fontSize:9,color:T.green,letterSpacing:"0.15em"}}>● ONLINE · ANÁLISE DE RECEITAS E INTERAÇÕES</div>
        </div>
      </div>
      <div style={{padding:"12px 28px",background:T.redBg,borderBottom:`1px solid ${T.red}20`,flexShrink:0}}>
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          ...
          <div style={{fontSize:12,color:T.inkMid,lineHeight:1.7}}>
            <strong style={{color:T.ink}}>Aviso:</strong> Rafael identificará interações e <strong style={{color:T.red}}>alertará o Dr. Dohmann</strong> em casos de risco. Em urgências, busque atendimento imediato.
          </div>
        </div>
      </div>
      <ChatIA membro="farmaceutico" apiKey={apiKey}
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{borderBottom:`1px solid ${T.border}`,padding:"16px 28px",display:"flex",alignItems:"center",gap:14,background:T.surface,flexShrink:0,boxShadow:T.shadowCard}}>
        <div style={{width:42,height:42,borderRadius:"50%",background:eq.bg,border:`1.5px solid ${eq.cor}40`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,boxShadow:T.shadowCard}}>{eq.icon}</div>
        <div>
          <div style={{fontFamily:T.fD,fontSize:20,color:T.ink}}>{eq.nome} · Geneticista Clínica</div>
          <div style={{fontSize:9,color:T.green,letterSpacing:"0.15em"}}>● ONLINE · ANÁLISE DE LAUDOS GENÉTICOS</div>
        </div>
      ...
      <div style={{borderBottom:`1px solid ${T.border}`,padding:"0 28px",display:"flex",background:T.bgWarm,flexShrink:0}}>
        {[{id:"risco",label:"Análise de Risco"},{id:"chat",label:"Falar com Dra. Clara"}].map(t=>(
          <button key={t.id} onClick={()=>setAbaGen(t.id)}
            style={{background:"none",border:"none",borderBottom:`2px solid ${abaGen===t.id?T.purple:"transparent"}`,padding:"13px 22px",fontSize:10,letterSpacing:"0.15em",textTransform:"uppercase",color:abaGen===t.id?T.purple:T.inkFaint,cursor:"pointer",fontFamily:T.fB,transition:"all 0.2s"}}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"28px"}}>
                ...
                {PAINEL_GENETICO.map((cat,i)=>(
                  <Card key={i} style={{overflow:"hidden"}}>
                    <div style={{padding:"14px 18px",borderBottom:`1px solid ${T.border}`,background:cat.bg,display:"flex",alignItems:"center",gap:12}}>
                      ...
                      <div>
                        <div style={{fontSize:13,color:cat.color,fontWeight:600}}>{cat.cat}</div>
                        <div style={{fontSize:9,color:T.inkFaint,marginTop:2}}>{cat.genes.length} GENES</div>
                      </div>
                    </div>
          ...
          {analisando&&(
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"60%",gap:20,textAlign:"center"}}>
              ...
              <div style={{fontSize:13,color:T.inkMid}}>Identificando variantes e cruzando com seu plano...</div>
            </div>
          )}
                    ...
                    <Card style={{overflow:"hidden"}}>
                      <div style={{padding:"20px 24px",borderBottom:`1px solid ${T.border}`,background:T.purpleBg,display:"flex",gap:14,alignItems:"flex-start"}}>
                        ...
                        {analise.nivel_risco_geral&&(
                          <div style={{marginLeft:"auto",padding:"6px 14px",borderRadius:20,background:impBg(analise.nivel_risco_geral),border:`1px solid ${impColor(analise.nivel_risco_geral)}40`,fontSize:10,color:impColor(analise.nivel_risco_geral),fontWeight:700,letterSpacing:"0.12em",flexShrink:0}}>
                            {analise.nivel_risco_geral.toUpperCase()}
                          </div>
                        )}
                      </div>
                      <div style={{padding:"14px 24px",background:T.bgWarm}}>
                        <div style={{fontSize:12,color:T.inkMid,lineHeight:1.7}}>💡 Faça suas perguntas na aba <strong>"Falar com Dra. Clara"</strong>.</div>
                      </div>
                    </Card>
                    {(analise.alertas||[]).map((a,i)=>{
                      const lv={critico:{c:T.red,bg:T.redBg,icon:"🚨"},atencao:{c:T.gold,bg:T.goldFaint,icon:"⚠️"},informativo:{c:T.blue,bg:T.blueBg,icon:"ℹ️"}}[a.nivel]||{c:T.blue,bg:T.blueBg,icon:"ℹ️"};
                      return (<Card key={i} style={{padding:"14px 18px",background:lv.bg,border:`1px solid ${lv.c}30`}}><div style={{display:"flex",gap:10}}><span style={{fontSize:18,flexShrink:0}}>{lv.icon}</span><span style={{fontSize:13,color:T.ink,lineHeight:1.7}}>{a.msg}</span></div></Card>);
                    })}
                          ...
                          {analise.achados.map((a,i)=>(
                            <div key={i} style={{padding:"14px 16px",background:impBg(a.impacto),border:`1px solid ${impColor(a.impacto)}30`,borderRadius:8}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                                ...
                                <span style={{fontSize:9,padding:"3px 9px",borderRadius:4,background:"rgba(255,255,255,0.7)",color:impColor(a.impacto),fontWeight:700,letterSpacing:"0.12em"}}>
                                  IMPACTO {(a.impacto||"").toUpperCase()}
                                </span>
                              </div>
                              <div style={{fontSize:11,color:T.inkMid,marginBottom:8}}>{a.variante}</div>
                              <div style={{fontSize:12,color:T.ink,lineHeight:1.6,borderTop:`1px solid rgba(0,0,0,0.06)`,paddingTop:8}}>
                                <span style={{color:impColor(a.impacto)}}>→ </span>{a.orientacao}
                              </div>
                            </div>
                          ))}
                        </div>
      ...
      {abaGen==="chat"&&(
        !pdfB64
          ?<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:20,padding:40,textAlign:"center"}} onClick={()=>fileRef.current?.click()}>
            ...
            <div style={{fontSize:13,color:T.inkFaint,maxWidth:400,lineHeight:1.8}}>A Dra. Clara responderá perguntas baseadas nos seus resultados reais.</div>
            <Btn variant="outline" style={{borderColor:T.purple,color:T.purple}}>SELECIONAR PDF →</Btn>
          </div>
          :<ChatIA key={chatKey} membro="geneticista" apiKey={apiKey} pdfB64={pdfB64}
            placeholder="Pergunte sobre seus resultados genéticos..."
...
function ModuloDocumentos({apiKey}){
  const fileRef = useRef(null);
      ...
      <div style={{width:280,flexShrink:0,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",overflow:"hidden",background:T.bgWarm}}>
        <div style={{padding:"16px"}}>
          <div
            ...
            onMouseOver={e=>{e.currentTarget.style.borderColor=T.gold;e.currentTarget.style.background=T.goldFaint;}}
            onMouseOut={e=>{e.currentTarget.style.borderColor=T.borderMid;e.currentTarget.style.background=T.surface;}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"0 12px 12px",display:"flex",flexDirection:"column",gap:8}}>
          {docs.length===0&&(
            <div style={{textAlign:"center",padding:"32px 16px"}}>
              ...
              <div style={{fontSize:12,color:T.inkFaint,lineHeight:1.6}}>Nenhum documento ainda.</div>
            </div>
          )}
          {docs.map(doc=>{
            const tm = TIPO_META[doc.tipo]||TIPO_META.outro;
            return (
              <div key={doc.id} onClick={()=>setSelDoc(doc.id)}
                style={{padding:"12px 14px",background:selDoc===doc.id?T.goldFaint:T.surface,border:`1.5px solid ${selDoc===doc.id?T.gold:T.border}`,borderRadius:8,cursor:"pointer",transition:"all 0.18s",boxShadow:T.shadowCard}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:5,alignItems:"center"}}>
                      <span style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:tm.bg,color:tm.color,fontWeight:700}}>{doc.tipo.toUpperCase()}</span>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"28px 32px"}}>
        {!docAtual&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:16,textAlign:"center"}}>
            ...
            <div style={{fontSize:13,color:T.inkFaint,maxWidth:360,lineHeight:1.8}}>A equipe HDohmann extrai diagnósticos, medicamentos, exames e ações de cada documento.</div>
          </div>
        )}
        {docAtual?.status==="analisando"&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"80%",gap:16,textAlign:"center"}}>
            ...
            <div style={{fontFamily:T.fD,fontSize:22,color:T.ink}}>Equipe analisando...</div>
          </div>
        )}
        {docAtual?.analise&&(
          <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:800,animation:"fadeUp 0.4s ease"}}>
            <Card style={{overflow:"hidden"}}>
              <div style={{padding:"18px 22px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:14,alignItems:"flex-start"}}>
                ...
                <div>
                  ...
                  {docAtual.analise.profissional&&<span style={{fontSize:11,color:T.inkFaint}}>Dr(a). {docAtual.analise.profissional}</span>}
                </div>
              </div>
              <div style={{padding:"16px 22px",background:T.goldFaint,display:"flex",gap:12,alignItems:"flex-start"}}>
                ...
                <div>
                  <Lbl color={T.gold}>Equipe HDohmann · Análise</Lbl>
            ...
            {(docAtual.analise.alertas||[]).map((a,i)=>{
              const lv={critico:{c:T.red,bg:T.redBg,icon:"🚨"},atencao:{c:T.gold,bg:T.goldFaint,icon:"⚠️"},informativo:{c:T.blue,bg:T.blueBg,icon:"ℹ️"}}[a.nivel]||{c:T.blue,bg:T.blueBg,icon:"ℹ️"};
              return (<Card key={i} style={{padding:"14px 18px",background:lv.bg,border:`1px solid ${lv.c}30`}}><div style={{display:"flex",gap:10}}><span style={{fontSize:16}}>{lv.icon}</span><span style={{fontSize:13,color:T.ink,lineHeight:1.6}}>{a.mensagem}</span></div></Card>);
            })}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              {(docAtual.analise.diagnosticos||[]).length>0&&(
                <Card style={{padding:"18px 20px"}}>
                  <Lbl color={T.purple}>Diagnósticos</Lbl>
                  {docAtual.analise.diagnosticos.map((d,i)=><div key={i} style={{fontSize:12,color:T.inkMid,padding:"6px 0",borderBottom:`1px solid ${T.border}`,lineHeight:1.5}}>◆ {d}</div>)}
                </Card>
              )}
              ...
              <Card style={{padding:"18px 20px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:10}}>
                  {docAtual.analise.checklist.map((item,i)=>(
                    <div key={i} style={{display:"flex",gap:10,padding:"10px 14px",background:T.bgWarm,borderRadius:8,border:`1px solid ${T.border}`}}>
    ...
    <div style={{flex:1,display:"flex",overflow:"hidden"}}>
      <div style={{flex:1,overflowY:"auto",padding:"24px 28px",background:T.bg}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:12}}>
          {INTEGRACOES.map(it=>{
            const conn=!!connected[it.id];
            return (
              <Card key={it.id} hover onClick={()=>setSelInt(it.id===selInt?null:it.id)}
                style={{padding:"18px",border:`1.5px solid ${selInt===it.id?T.gold:conn?T.green+"60":T.border}`,background:selInt===it.id?T.goldFaint:T.surface}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:5}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:conn?T.green:T.surfaceMid,boxShadow:conn?`0 0 7px ${T.green}60`:"none"}}/>
                    <span style={{fontSize:9,color:conn?T.green:T.inkFaint,letterSpacing:"0.1em"}}>{conn?"CONECTADO":"OFFLINE"}</span>
                  </div>
                </div>
                <div style={{fontSize:13,color:T.ink,fontWeight:600,marginBottom:6}}>{it.nome}</div>
                <div style={{display:"flex",gap:4,marginBottom:8}}>
                  {it.plat.map(p=><span key={p} style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:p==="iOS"?T.blueBg:T.greenBg,color:p==="iOS"?T.blue:T.green,fontFamily:T.fB}}>{p}</span>)}
                </div>
      ...
      {item&&(
        <div style={{width:360,flexShrink:0,borderLeft:`1px solid ${T.border}`,display:"flex",flexDirection:"column",overflow:"hidden",background:T.surface,boxShadow:"-2px 0 12px rgba(60,50,30,0.04)"}}>
          <div style={{padding:"20px 22px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:15,color:T.ink,fontWeight:600}}>{item.nome}</div>
              <div style={{display:"flex",gap:4,marginTop:4}}>
                {item.plat.map(p=><span key={p} style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:p==="iOS"?T.blueBg:T.greenBg,color:p==="iOS"?T.blue:T.green,fontFamily:T.fB}}>{p}</span>)}
              </div>
            </div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"20px 22px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:10}}>
              {item.passos.map((p,i)=>{
                const done=isStepDone(item.id,i);
                return (
                  <div key={i} onClick={()=>toggleStep(item.id,i)}
                    style={{display:"flex",gap:10,padding:"12px 14px",background:done?T.greenBg:T.bgWarm,border:`1.5px solid ${done?T.green+"50":T.border}`,borderRadius:8,cursor:"pointer",transition:"all 0.2s",boxShadow:T.shadowCard}}>
                    <div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${done?T.green:T.borderMid}`,background:done?T.green:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1,transition:"all 0.2s"}}>
                      {done?<span style={{fontSize:10,color:"#FFF",fontWeight:700}}>✓</span>:<span style={{fontSize:9,color:T.inkFaint}}>{i+1}</span>}
                    </div>
              ...
              {allDone(item.id)&&!connected[item.id]
                ...
                :connected[item.id]
                  ...
                  :<Card style={{padding:"14px 16px",background:T.goldFaint,border:`1px solid ${T.goldBorder}`}}>
                    <div style={{fontSize:12,color:T.inkMid,lineHeight:1.6,marginBottom:10}}>Complete todos os passos acima.</div>
                    <div style={{display:"flex",gap:6,alignItems:"center"}}>
                      <div style={{height:3,flex:1,background:T.surfaceMid,borderRadius:2,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${(item.passos.filter((_,idx)=>isStepDone(item.id,idx)).length/item.passos.length)*100}%`,background:T.gold,transition:"width 0.3s ease"}}/>
                      </div>
  ...
  return (
    <div style={{display:"flex",height:"100vh",background:T.bg,fontFamily:T.fB,color:T.ink,overflow:"hidden"}}>
      ...
      <div style={{width:220,flexShrink:0,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",background:T.surface,height:"100vh",position:"sticky",top:0,overflow:"hidden",boxShadow:"2px 0 12px rgba(60,50,30,0.04)"}}>
        ...
        <div style={{padding:"12px 16px 10px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:10,alignItems:"center"}}>
          ...
          <div style={{minWidth:0}}>
            <div style={{fontSize:12,color:T.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:500}}>{nome.split(" ")[0]}</div>
            <div style={{fontSize:9,color:T.inkFaint}}>{form?.cargo||"Executivo"}</div>
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"10px 10px"}}>
          ...
          {MODULOS.map(m=>{
            ...
            return (
              <button key={m.id} onClick={()=>setModulo(m.id)}
                style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:8,background:active?T.goldFaint:"transparent",border:`1px solid ${active?T.goldBorder:"transparent"}`,cursor:"pointer",transition:"all 0.18s",fontFamily:T.fB,textAlign:"left",marginBottom:2,boxShadow:active?T.shadowCard:"none"}}
          ...
          {EQUIPE.map(e=>(
            <div key={e.id} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 12px",borderRadius:8}}>
              ...
              <div style={{minWidth:0}}>
                <div style={{fontSize:11,color:T.inkMid,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.nome}</div>
                <div style={{fontSize:8,color:T.inkFaint,letterSpacing:"0.08em"}}>{e.titulo}</div>
              </div>
        ...
        <div style={{padding:"14px 16px",borderTop:`1px solid ${T.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{fontSize:10,color:T.inkFaint}}>VITALIDADE</span>
            <span style={{fontSize:13,color:T.gold,fontFamily:T.fD,fontWeight:700}}>{scores.total}/100</span>
          </div>
          <div style={{height:3,background:T.surfaceMid,borderRadius:2,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${scores.total}%`,background:`linear-gradient(90deg,${T.gold},${T.teal})`}}/>
          </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {modulo==="dashboard"    && <ModuloDashboard form={form} scores={scores} setModulo={setModulo}/>}
        {modulo==="plano"        && <ModuloPlano form={form} scores={scores} setModulo={setModulo}/>}
        ...
        {modulo==="documentos"   && <ModuloDocumentos apiKey={apiKey}/>}
